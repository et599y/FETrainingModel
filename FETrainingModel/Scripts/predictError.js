...
function predict_error(file) {
    d3.json(file, function (error, data) {
        ...
        d3.selectAll(".select").on("change", function (d) {
            ...
            updateSvg(selectCol.text(), index);//selectCol.property("value")
        })

        // Add X axis --> it is a date format
        var x9 = d3.scaleLinear()
            .domain(d3.extent([0, data[index].predict_error.length - 1]))
            .range([0, width9]);
        ...
        var y9 = d3.scaleLinear()
            .domain([d3.min(data[index].predict_error), d3.max(data[index].predict_error)])
            .range([height9, 0]);
        ...
        // Add the line
        line.append("path")
            .datum(data[index].predict_error)//data
        ...
        function updateSvg(selectOption, index, error) {
            y9.domain([d3.min(data[index].predict_error), d3.max(data[index].predict_error)])
            x9.domain(d3.extent([0, data[index].predict_error.length - 1]))
            yAxis9.transition().duration(1000).call(d3.axisLeft(y9))
            xAxis9.transition().duration(1000).call(d3.axisBottom(x9))

            line
                .select('.line1')
                .datum(data[index].predict_error)
                .transition()
                .duration(1000)
                .attr("d", d3.line()
                    .x(function (d, i) { return x9((i)) })
        ...
        function mousemove9() {
            ...
            selectedData = data[index].predict_error[Math.round(i)]
            focus9
                .attr("cx", x9(i))
