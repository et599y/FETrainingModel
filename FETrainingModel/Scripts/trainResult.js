...
function result_train(file) {
    d3.json(file, function (error, data) {
        ...
        var x11 = d3.scaleLinear()
            .domain(d3.extent([0, data[index].y_train.length - 1]))
            .range([0, width11]);
        xAxis11 = svg11.append("g")
            .attr("transform", "translate(0," + height11 + ")")
            .call(d3.axisBottom(x11));

        var valArray = data[index].y_train.concat(data[index].predict_train);
        // Add Y axis
        ...
        line.append("path")
            .datum(data[index].predict_train)
        ...
        function mousemove1() {
            ...
            //
            selectedData = data[index].y_train[Math.round(i)]
            selectedData2 = data[index].predict_train[Math.round(i)]
            focus11
                .attr("cx", x11(i))
            ...
            tooltip1
                ...
                .html("y_train: " + selectedData + "<br>" + "pred_train: " + selectedData2)
                .style("opacity", 1);
        }
        function mouseout1() {
        ...
        svg11.on("dblclick", function () {
            x11.domain(d3.extent([0, data[index].y_train.length - 1]))
        ...
        svg11.append("text").attr("x", 960).attr("y", 0).text("y_train").style("font-size", "15px").attr("alignment-baseline", "middle")
        svg11.append("text").attr("x", 960).attr("y", 20).text("pred_train").style("font-size", "15px").attr("alignment-baseline", "middle")
    })
}
